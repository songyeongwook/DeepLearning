...
class RNN(nn.Module):
    def __init__(self, dimension, hidden_size, num_layers, num_classes, device):
        ...
        self.rnn = nn.RNN(self.dimension, self.hidden_size, self.num_layers, batch_first=True)#bidirectional=True) 출력값에서 역으로 작동
        self.fc = nn.Linear(self.hidden_size, self.num_classes)
    
    def forward(self, x):
        # (num_layers, batch_size, hidden_size)
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(self.device) #없어도 됨 알아서 생성해줌
        
        # RNN forward: out shape = (batch_size, sequence_length, hidden_size)
        out, h_out = self.rnn(x, h0)
        
        # Use the output from the last time step
        out = self.fc(out[:, -1, :]) # many to many -> many to one이면 결국 모두다 계산하고 마지막에 몇개 뽑아서 쓸건지 나타냄
