...
def load_mnist(use_small: bool=False,  valDB_portion: float=0.0):
    # 1) MNIST npz 로드 및 정규화 (0~1 범위로)
    mnist   = np.load('mnist.npz')
    x_train = mnist['x_train'].astype(np.float32) / 255.0
    y_train = mnist['y_train']
    x_test  = mnist['x_test'].astype(np.float32) / 255.0
    y_test  = mnist['y_test']
